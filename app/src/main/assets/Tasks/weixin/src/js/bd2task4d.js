;(function(win, doc, undefined) {
    var Task = angular.module('BD2Task4D', ['ngRoute', 'ngTouch', 'w5c.validator', 'AppModule']);
    ...
    Task.config(['$routeProvider',
        function($routeProvider) {
            $routeProvider.
            when('/d3list/', {
                templateUrl: 'views/bdtask/d3.list.4bd2.html',
            ...
            when('/d2list/', {
                templateUrl: 'views/bdtask/d2.list.html',
    ...
    Task.controller('DxFormCtrler', ['$scope', '$routeParams', '$http', '$location', '_loading', '_pageTitle', '_wxService', '_APP', '_jsTree',
        function($scope, $routeParams, $http, $location, _loading, _pageTitle, _wxService, _APP, _jsTree) {
            ...
            $scope.getSaleTree = function(){
                _jsTree.init('test/json/BDTask/GetSaleTree.json?pid='+$scope.pd.id,function(nodes){
                    var district = [];
                    var districtStr = [];
                    for(var i=0,l=nodes.length;i<l;i++){
                        district.push(nodes[i].original);
