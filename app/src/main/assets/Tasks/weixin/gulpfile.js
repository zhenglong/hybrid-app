...
var config = require(('legacy' in util.env) ? './config-legacy' : './config'); //加载我们的配置文件
...
var jqMobileCss = paths.jqMobileCss;
var jqMobileJs = paths.jqMobileJs;
var sectionJavascript = paths.sectionJavascript;
var apiReg = paths.api.reg;
var apiRep = paths.api.rep;
var timestamp = new Date();
var revTemp = (new Date().valueOf()) + '';
var rev = paths.release.rev ? '-'+version+'.'+ revTemp : '';
var viewReg = /(templateUrl\s*:\s*[\'\"])((\.{1,2}\/)*views)\//g;
var viewRep = '$1$2'+rev+'/';
...
gulp.task('src:inject', ['src:less', 'src:sass'], function() {
    ...
    var sourcesJqMobileCss = gulp.src(jqMobileCss, {
        read: false
    });
    var sourcesJqMobileJs = gulp.src(jqMobileJs, {
        read: false
    });
	...
	var sources_sectionJavascript = gulp.src(sectionJavascript, {
		read: false
	});
    ...
    return target.pipe(inject(sources_libcss, {
		...
		.pipe(inject(sources_sectionJavascript, {
			relative: true,
			starttag: '<!-- inject:sectionJavascript -->'
		}))
        .pipe(inject(sources_libjs, {
            relative: true,
            starttag: '<!-- inject:lib:js -->'
        }))
        .pipe(inject(sourcesJqMobileJs, {
            relative: true,
            starttag: '<!-- inject:jqmobile:js -->'
        }))
        .pipe(inject(sourcesJqMobileCss, {
            relative: true,
            starttag: '<!-- inject:jqmobile:css -->'
        }))
...
//压缩jqMobile css
gulp.task('release:minJqMobileCss', /*['release:fonts'],*/ function() {
    return gulp.src(jqMobileCss)
        .pipe(minifyCSS())
        .pipe(concat('jqmobile.css'))
        .pipe(rename(function (path) {
            path.basename += rev;
        }))
        .pipe(gulp.dest(dir_release + 'css/'));
...
//压缩 css(总)
gulp.task('release:css', ['release:minLibCss', 'release:minJqMobileCss', 'release:minPageCss']);
...
//压缩 jqMobile js
gulp.task('release:minJqMobileJs', function() {
    return gulp.src(jqMobileJs)
        .pipe(replace(apiReg, apiRep)) //替换api路径
        .pipe(replace(viewReg, viewRep)) //替换view rev
        .pipe(sourcemaps.init())
        .pipe(uglify())
        .pipe(concat('jqmobile.js'))
        .pipe(rename(function (path) {
            path.basename += rev;
        }))
        .pipe(sourcemaps.write('maps/'))
...
// 压缩 js(总)
gulp.task('release:js', ['release:minLibJs', 'release:minJqMobileJs', 'release:minPageJs']);

//压缩CSS引用图片
gulp.task('release:minHtmlImg', ['clear'], function() {
...
//压缩 html中的图片
gulp.task('release:minCssImg', ['clear'], function() {
...
gulp.task('release:inject', ['release:css', 'release:js'], function() {
    var target = gulp.src(['src/**/*.html', '!src/views/**/*.html', '!src/test/**/*.html']).pipe(gulp.dest(dir_release + ''));
    ...
    var sourcesJqMobileCss = gulp.src(dir_release + 'css/jqmobile'+rev+'.css', {
        read: false
    });
    var sourcesJqMobileJs = gulp.src(dir_release + 'js/jqmobile'+rev+'.js', {
        read: false
    });
	...
	var sources_sectionJavascript = gulp.src(dir_release + 'js/common/data' +rev+ '.js', {
		read: false
	});
...
//压缩 view html
gulp.task('release:minViewHtml', function() {
    var opts = {
        empty: true,
        conditionals: true,
        spare: true,
...
//带html压缩的发布(总)
gulp.task('release:+minhtml', ['release', 'release:html']);
