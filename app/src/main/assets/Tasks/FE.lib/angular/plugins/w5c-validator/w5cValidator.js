...
angular.module("w5c.validator", ["ng"])
    .provider('w5cValidator', [function () {
        var defaultRules = {
            ...
            elemTypes = ['text', 'password', 'email', 'number', 'url','tel', 'hidden', ['textarea'], ['select'], ['select-multiple'], ['select-one']];
        ...
        var validatorFn = function () {
            ...
            this.defaultShowError = function (elem, errorMessages) {
                ...
                if (!this.isEmpty($group) && !$group.hasClass("has-error")) {
                    ...
                    $group.append('<small class="text-danger w5c-error">' + errorMessages[0] + '</small>');
                }
            };
...
angular.module("w5c.validator")
    ...
    .directive("w5cLt", [function () {
        'use strict';
        return {
            require: "ngModel",
            link   : function (scope, elem, attrs, ctrl) {
                var otherInput = elem.inheritedData("$formController")[attrs.w5cLt];
                ctrl.$parsers.push(function (value) {
                    ...
                    ctrl.$setValidity("lt", false);
                });

                otherInput.$parsers.push(function (value) {
                    ctrl.$setValidity("lt", value > ctrl.$viewValue);
                    return value;
                });
            }
        };
    }])
    .directive("w5cGt", [function () {
        'use strict';
        return {
            require: "ngModel",
            link   : function (scope, elem, attrs, ctrl) {
                var otherInput = elem.inheritedData("$formController")[attrs.w5cGt];
                ...
                ctrl.$parsers.push(function (value) {
                    ...
                    ctrl.$setValidity("gt", false);
                });

                otherInput.$parsers.push(function (value) {
                    ctrl.$setValidity("gt", value < ctrl.$viewValue);
                    return value;
                });
            }
        };
